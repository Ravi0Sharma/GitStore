...
gitCommand
  ...
  .action(async (options) => {
    try {
      ...
      console.log('\n📊 Git Status:');
      console.log(`Current branch: ${status.current || 'N/A'}`);
      ...
      console.log(`\nUntracked files:`);
      const untrackedFiles = status.files?.filter((f: any) => f.working_dir === '??') || [];
      if (untrackedFiles.length > 0) {
        untrackedFiles.forEach((file: any) => console.log(`  ? ${file.path || file}`));
      } else {
        console.log('  (none)');
      }
    } catch (error) {

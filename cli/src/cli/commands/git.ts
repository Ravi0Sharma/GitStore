...
gitCommand
  ...
  .action(async (options) => {
    try {
      ...
      const status = await handleGitOperation(
        absoluteRepoPath,
        async (git) => git.status(),
        'status'
      );
      ...
      if (status.staged.length > 0) {
        status.staged.forEach((file: string) => console.log(`  ✓ ${file}`));
      } else {
        console.log('  (none)');
      }
      console.log(`\nChanges not staged for commit:`);
      if (status.not_added.length > 0) {
        status.not_added.forEach((file: string) => console.log(`  ✗ ${file}`));
      } else {
        console.log('  (none)');
      }
      console.log(`\nUntracked files:`);
      const untrackedFiles = status.files?.filter((f: any) => f.working_dir === '??').map((f: any) => f.path) || [];
      if (untrackedFiles.length > 0) {
        untrackedFiles.forEach((file: string) => console.log(`  ? ${file}`));
      } else {
        console.log('  (none)');
      }
    } catch (error) {
...
gitCommand
  ...
  .action(async (options) => {
    try {
      ...
      const result = await handleGitOperation(
        absoluteRepoPath,
...
gitCommand
  ...
  .action(async (options) => {
    try {
      ...
      const log = await handleGitOperation(
        absoluteRepoPath,
        async (git) => git.log({ maxCount: number }),
        'log'
      );
